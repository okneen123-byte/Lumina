...
def signup(data: SignupModel):
    if not create_user(data.email.lower(), data.password):
...
def news(req: NewsRequest):
    ...
    articles = get_news(req.category, req.language, req.sort_by, req.limit)
    return {"category": req.category, "language": req.language, "news": articles}
